...
public class PlayerControlsDemoActivity extends YouTubeFailureRecoveryActivity implements
    ...
    private static final ListEntry[] ENTRIES = {
            new ListEntry("Playlist: Google I/O 2012", "y7ECYaMAUKE", false)};
...
//    skipTo.setOnEditorActionListener(this);
        ...
        play_status = (TextView) findViewById(R.id.play_status);
        mute_status = (TextView) findViewById(R.id.volume);
...
//TODO  change the code here to check if the player is running or not, and perform action accordingly
    ...
    public void play_pause(View view) {
        if (play_status.getText().equals("playing")) {
            player.pause();
        } else if (play_status.getText().equals("paused")) {
            player.play();
        }
    }
    ...
    public void mute(View view) {
        ...
        Log.i("mute", "mute press detected");
        Log.i("mute", mute_status.getText().toString());

        if(mute_status.getText().equals("unmuted")) {

            AudioManager audioManager = (AudioManager) PlayerControlsDemoActivity.this.getSystemService(Context.AUDIO_SERVICE);
            audioManager.setRingerMode(AudioManager.RINGER_MODE_SILENT);
            mute_status.setText("muted");


        }
        else if(mute_status.getText().equals("muted"))
        {
            AudioManager audioManager = (AudioManager)PlayerControlsDemoActivity.this.getSystemService(Context.AUDIO_SERVICE);
            audioManager.setRingerMode(AudioManager.RINGER_MODE_NORMAL);
            mute_status.setText("unmuted");
        }
    }
...
//    pauseButton.setEnabled(enabled);
    ...
    private final class MyPlaybackEventListener implements PlaybackEventListener {
        ...
        public void onPlaying() {
            ...
            //  updateText();
            play_status.setText("playing");
            mute_status.setText("unmuted");
            Log.i("Main", "playing started");
